...
function genererRecuperation(result) {
  ...
  const texte = charge < 5
    ? "Séance légère — tu peux t'entraîner demain."
    : charge < 15
    ? "Séance modérée — 1 à 2 jours de récupération active."
    : charge < 30
    ? "Séance intense — 2 jours de repos. Dors bien et hydrate-toi."
...
const conseilsPersonnalises = {
  ...
  "vélo":   ["🚴 Pédalage mono-jambe 30s/jambe.", "🚴 2×20min à 85-90% FCmax (travail au seuil).", "🚴 60g de glucides/heure pour efforts > 1h.", "🚴 Un fitting vélo peut t'apporter 5-10% de puissance.", "🚴 Alterne position assise et danseuse."],
...
/* =========================
   AUTH — CORRECTION BUG CONNEXION
   On ne réinjecte plus le HTML du formulaire.
   On garde le HTML fixe dans index.html et on
   attache les events une seule fois au DOMContentLoaded.
========================= */
...
/* =========================
   INIT — events attachés UNE SEULE FOIS
========================= */

window.addEventListener("DOMContentLoaded", () => {

  // Analyser / Réinitialiser
  el("analyzeBtn")?.addEventListener("click", analyser);
  el("resetBtn")?.addEventListener("click", reinitialiser);

  // Inscription
  el("signupBtn") || el("inscriptionBtn")?.addEventListener("click", async () => {
